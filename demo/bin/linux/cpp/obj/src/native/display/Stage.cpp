...
Void Stage_obj::nmeOnKey( Dynamic inEvent,::String inType){
{
		...
		if (((stack->length > (int)0))){
			...
			if (((bool((bool((((int(flags) & int(::native::display::Stage_obj::efAltDown))) > (int)0)) && bool((inEvent->__Field(HX_CSTRING("result"),true) != (int)-1)))) && bool((inEvent->__Field(HX_CSTRING("code"),true) == ::native::ui::Keyboard_obj::ENTER))))){
				HX_STACK_LINE(573)
				if (((this->get_displayState() == ::native::display::StageDisplayState_obj::NORMAL_dyn()))){
					HX_STACK_LINE(575)
					this->set_displayState(::native::display::StageDisplayState_obj::FULL_SCREEN_INTERACTIVE_dyn());
				}
				else{
					HX_STACK_LINE(579)
					this->set_displayState(::native::display::StageDisplayState_obj::NORMAL_dyn());
				}
			}
		}
	}
